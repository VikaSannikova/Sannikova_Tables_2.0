...
{
	Data data; //данные 
	NodeTree* left; //указатель на левый
	NodeTree* right; //указатель на правый

	NodeTree() : left(0), right(0) {} //конструктор
	NodeTree(const Data &d) //конструктор
	{
		data = d;
		left = 0;
		right = 0;
	}
	NodeTree(const NodeTree<Data> &n) //конструктор копирования
	{
		data = n.data;
		left = n.left;
		right = n.right;
...
private:
	...
	NodeTree<TableNode<Key, Data>> *root; //указатель на корень
...
public:
	...
	Data& operator [](Key k) //перегрузка инициализации
	{
		...
		NodeTree<TableNode<Key, Data>>*pred = 0, *tmp = root; //узлы дерева
		int i;
		...
		tmp = new NodeTree<TableNode<Key, Data>>(); //тмп пустой новый узел
	...
	NodeTree<TableNode<Key, Data>>*find(Key k) //поиск элемента по ключу

	{
		Data t(0);
		TableNode<Key, Data> el(k, t); //создаем элемент с такими данными
		NodeTree<TableNode<Key, Data>> *tmp = root; //указатель на корень
	...
	void printBFS() //печать дерева
	{
		...
		queue<NodeTree<TableNode<Key, Data>>*> q; //очередь из листов
		if (root) //если существует корень
			q.push(root); //то добавляем корень
		while (!q.empty()) //пока очередь не пуста
		{
			NodeTree<TableNode<Key, Data>> *tmp = q.front(); //первый элемент в очереди
